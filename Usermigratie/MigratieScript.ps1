...
dienen voldoende rechten te hebben op de Exchange omgeving.


File Name  : renameExistingUser.ps1
...
function memberOf($gebruikersnaam,$oldGroup,$newGroup){
    ...
    {
        Write-Host "$gebruikersnaam is lid van $oldGroup"
...
try{
    ...
    Write-Host "Fout bij het verbinden met Exchange. Script sluit zichzelf af"
...
try{
    #default locatie "\\fileserver\Iva\Scripts"
    $inputfile = Get-FileName "\\fileserver\Iva\Scripts"
    $UserList = Import-Csv $inputfile
}
catch{
    Write-Host "Geen bestand ingegeven voor gebruikers. Script sluit zichzelf af"
    throw "Geen bestand ingegeven voor gebruikers. Script sluit zichzelf af"
}
#endregion

#region creeren nieuwe gebruikersnaam op basis van oude gebruikersnaam
#alle users overlopen in CSV file
foreach ($oldUser in $UserList)
{
    try{
        ...
        Write-Host "Fout bij het ophalen van de oude gebruiker"
...
#region aanvullen achternaam indien deze minder dan 4 letters bevat
        ...
        Write-Host "Probleem bij het aanvullen van de gebruikersnaam voor iemand met korte naam"
...
#aanmaken gebruikersnaam door voor- en achternaam te combineren in hoofdletters
    try{
        ...
        Write-Host "Fout bij het aanmaken van de gebruikersnaam"
...
#als deze reeds bestaat laatste letter verwijderen en vervangen door 1-9
    ...
    try{
        ...
        Write-Host "Fout bij het controleren of de gebruikersnaam nog beschikbaar is"
...
#effectief aanmaken van de gebruiker op basis van de aangemaakte gebruikersnaam
    ...
    #ophalen loginscript en printscript van oude gebruiker
        ...
        Write-Host "Printscript gekopieerd"
        ...
        Write-Host "Loginscript gekopieerd"


    }
    catch{
        Write-Host "Probleem bij het aanpassen van het loginscript van de gebruiker. Gelieve deze zelf te kopieren"
    }
    ...
    #proberen effectief aanmaken gebruiker
        if($user.otherTelephone){
            ...
            -Enabled $true -PasswordNeverExpires $false -Initials ($user.GivenName.Substring(0,1) + $user.sn.Substring(0,1)) `
        ...
        Write-Host "Er was een probleem bij het aanmaken van de gebruiker"
    ...
    #region citrix groepen in orde brengen
    ...
    memberof $username 'CTX_PRD_SET_ALLOW_AUDIO' 'CTX_PRD_XA7X_SET_ALLOW_CLIENT_DRIVES_ALL'
...
#activesync toestellen ophalen
    ...
    try{
        ...
        Write-Host "Er was een probleem bij ophalen van de mailboxdatabase van de gebruiker"
    }
...
"
                ...
                Write-Host "Email gestuurd naar aanvraag BPM-ICT voor configuratie smartphone"
            }
        }
    }
    catch{
        Write-Host "Probleem bij het ophalen van activeSync toestellen"
    }
#endregion
...
"
                Send-MailMessage -From $From -to $to -Subject $Subject -Body $mail -SmtpServer $SMTPServer -BodyAsHtml -Encoding $encoding
                Write-Host "Email gestuurd naar Steven Moerman voor het aanpassen van de Faxaccount"
            }
            else{
        ...
        Write-Host "Probleem bij het ophalen van de proxyadressen van de oude gebruiker"
    }
#endregion

#region mailbox overzetten naar nieuwe gebruiker
    try{
        ...
        #Get-MailboxDatabase | Clean-MailboxDatabase
        Write-Host "Oude mailbox disabled"
        ...
        Write-Host "Nieuwe mailbox enabled"
        ...
        Write-Host "Emailadressen gekopieerd"

        foreach ($identity in $mailboxRrechten){
        ...
        Write-Host "probleem bij het afkoppelen/verbinden van de mailbox"
    }
#endregion
#endregion
...
"
    ...
    Remove-ADGroupMember $group $user.name -Server $server -Confirm:$False
...
#region uitschakelen oude gebruiker
#Disable AD account old user
    try{
        ...
        Write-Host "Probleem bij uitschakelen oude gebruikersaccount"
    }
...
#Scripts van oude gebruiker verplaatsen naar archief
    if ($user.ScriptPath){
        try{
            ...
            Write-Host "Oud loginscript verplaatst naar archief"
        }
        catch{
            Write-Host "Probleem bij verplaatsen van oud loginscript naar archief"
        }

        try{
            Move-Item -Path $printscript -Destination "\\fileserver\loginscript\Archief\"
            Write-Host "Oud printscript verplaatst naar archief"
        }
        catch{
        Write-Host "Probleem bij verplaatsen van oud printscript naar archief"
        }
    }
}
#endregion
